...
#import "SBBSageNetworkManager.h"
#import "SBBAppleCore.h"

@interface SBBSageNetworkManager ()
{
...
@implementation SBBSageNetworkManager
...
-(void)authenticateWithExistingCredentialsIfNecessaryWithSuccess:(void (^)(NSURLSessionDataTask *, id))success failure:(void (^)(NSURLSessionDataTask *, NSError *))failure
{
    ...
    else
    {
        ...
        if (!credential) {
            if (failure) {
                failure(nil,[NSError SBBNotAuthenticatedError]);
            }
        }
        else
        {
